						...
						try {
							...
							if (
								...
								throw new ServerUnreachableError(
									`Connection could not be established to your server running on port 8080. You may want to check your network, firewall, and other settings to try to resolve the issue.`,
								);
							} else {
					...
					async () => {
						try {
							...
							throw new ResourceUnreachableError(
								`The resource at /v1/contacts could not be reached. Make sure your routes are defined properly.`,
							);
						}
					},
				],
					...
					async () => {
						try {
							...
							throw new InvalidContentTypeError(
								`It looks like your API contacts resource isn't properly setting the Content-Type header to "application/json". Make sure you are using res.json() method in your Express application to set the response type to JSON.`,
							);
						}
					},
				],
				[
					...
					async () => {
						try {
							...
							throw new ApiTestingError(
								`The results that were returned were was not in the form of an Array`,
							);
						}
					},
				],
					...
					async () => {
						try {
							...
							throw new InvalidContactSchemaError(
								`One or more contacts returned in your contacts API resource did not contain a valid contact schema containing all of the required fields (id, fname, lname, phone, email, birthday). Check all of your contacts being returned to make sure their feilds match the required schema defined in this activity.`,
							);
						}
					},
				],
					...
					async () => {
						try {
							...
							if (
								...
								throw new ApiTestingError(
									`No body data was returned from GET /v1/contacts, or another unexpected error happened when attempting to hit the resource path with default pagination parameters. Please check your API's pagination logic.`,
								);
							...
							if (error.name === "UndefinedDataInResponseError")
								throw error;
							else if (
								error instanceof PaginationResultCountError
							)
								throw error;
							else
								throw new PaginationResultCountError(
									`There seems to more, or less than exactly 10 contacts that were returned in your results when there are no pagination parameters defined. The paginator should default to 10 contacts at a time.`,
								);
						}
					},
				],
				[
					...
					async () => {
						...
						const { data: dataOne } = await axios.get(
							`http://localhost:8080/v1/contacts?page=4&limit=5`,
						);
						expect(dataOne.length).toBe(5);

						const { data: dataTwo } = await axios.get(
							`http://localhost:8080/v1/contacts?page=2&limit=7`,
						);
		...
		[
			...
			[
				[
					...
					async () => {
						expect.assertions(6);
						const { data: lnameAsc } = await axios.get(
							"http://localhost:8080/v1/contacts?sort=lname&direction=asc",
						);
						const { data: fnameAsc } = await axios.get(
							"http://localhost:8080/v1/contacts?sort=fname&direction=asc",
						);
						const { data: lnameDesc } = await axios.get(
							"http://localhost:8080/v1/contacts?sort=lname&direction=desc",
						);
						...
						const filters = [
							{
								...
								by: "birthday",
								value: "1997-04-06",
								expect: { first: 67, last: 251 },
							},
							{
								op: "gt",
								by: "birthday",
								value: "1997-04-06",
								expect: { first: 251, last: 251 },
							},
							{
								op: "eq",
								by: "birthday",
								value: "1982-04-17",
								expect: { first: 243, last: 243 },
							},
							{
								op: "lte",
								by: "birthday",
								value: "1997-04-06",
								expect: { first: 3, last: 75 },
							},
							{
								op: "lt",
								by: "birthday",
								value: "1997-04-06",
								expect: { first: 3, last: 83 },
							},
						];
		...
		[
			...
			[
				...
				[
					...
					async () => {
						const { data } = await axios.get(
							"http://localhost:8080/v1/contacts/11",
						);
						expect.assertions(2);
					...
					async () => {
						...
						try {
							const update = await axios.put(
								"http://localhost:8080/v1/contacts/324",
								data,
							);
						} catch (error) {}
				...
				[
					...
					async () => {
						try {
							await axios.delete(
								"http://localhost:8080/v1/contacts/325",
							);
